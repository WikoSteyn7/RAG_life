...
def split_table(table, MAX_SECTION_LENGTH):
    header = re.search(r"<thead>.*?</thead>", table, re.DOTALL)
    header = header.group() if header else ""
    rows = re.findall(r"<tr>.*?</tr>", table, re.DOTALL)
    
    ROW_SEARCH_LIMIT = 512
    
    start_row = 0
    while start_row < len(rows):
        end_row = start_row
        table_part_length = len(header)
        search_limit = min(end_row + ROW_SEARCH_LIMIT, len(rows))
        while end_row < search_limit and table_part_length + len(rows[end_row]) <= MAX_SECTION_LENGTH:
            table_part_length += len(rows[end_row])
            end_row += 1

        if end_row == start_row:
            end_row = start_row + 1

        table_part = "".join(["<table>", header, "".join(rows[start_row:end_row]), "</table>"])
        yield table_part
        start_row = end_row

def split_text(page_map, filename):
    MAX_SECTION_LENGTH = 1280
    MIN_SECTION_LENGTH = 512
    SECTION_OVERLAP = 50
    SENTENCE_ENDINGS = {".", "!", "?"}
    SENTENCE_SEARCH_LIMIT = 512  # Define a limit to search for sentence endings
    MIN_LAST_SECTION_LENGTH = 20  # Define a minimum length for the last section
    ...
    for _, offset, page_text in page_map:
        length = len(page_text)
        page_end = offset + length
        start = offset
        while start < page_end:
            end = min(start + MAX_SECTION_LENGTH - SECTION_OVERLAP, page_end)
            
            # Search for sentence endings within the SENTENCE_SEARCH_LIMIT
            search_limit = min(end + SENTENCE_SEARCH_LIMIT, page_end)
            while end < search_limit and page_text[end - offset] not in SENTENCE_ENDINGS:
                end += 1
            end = min(end + 1, page_end)
            
            # Adjust overlap_end to end at a sentence boundary
            overlap_end = min(end + SECTION_OVERLAP, page_end)
            overlap_search_limit = min(overlap_end + SENTENCE_SEARCH_LIMIT, page_end)
            while overlap_end < overlap_search_limit and page_text[overlap_end - offset] not in SENTENCE_ENDINGS:
                overlap_end += 1
            overlap_end = min(overlap_end + 1, page_end)
            
            section_text = page_text[start - offset:end - offset]
            overlap_text = page_text[end - offset:overlap_end - offset]
            section_text += overlap_text

            # Process tables
            tables = re.findall(r"<table.*?</table>", section_text, re.DOTALL)
            if tables:
                processed_length = 0
                for table in tables:
                    table_start = section_text.find(table) + processed_length
                    table_parts = list(split_table(table, MAX_SECTION_LENGTH))
                    for part in table_parts:
                        yield (part, find_page(start + table_start))
                        processed_length += len(part)
                processed_length += table_start

                start += processed_length
                section_text = section_text[processed_length:]
            
            if len(section_text) >= MIN_SECTION_LENGTH or start + MIN_SECTION_LENGTH >= page_end:
                if len(section_text) < MIN_LAST_SECTION_LENGTH:
                    # Do not yield the last section if its length is shorter than MIN_LAST_SECTION_LENGTH
                    break
...
def create_sections(filename, page_map, use_vectors, embedding_deployment: str = None, embedding_model: str = None):
    ...
    output_file_path = os.path.join(output_dir, f"{file_id}.txt")
    
    for i, (content, pagenum) in enumerate(split_text(page_map, filename)):
        print(f"{content} + {pagenum} \n\n")
        section = {
            "id": f"{file_id}-page-{i}",
            "content": content,
        ...
        # Append the section content to the output text file
        with open(output_file_path, "a", encoding="utf-8") as output_file:
            output_file.write(f"Section ID: {section['id']}\n")
            output_file.write(f"Content: {section['content']}\n")
            output_file.write(f"Page Number: {pagenum}\n")
